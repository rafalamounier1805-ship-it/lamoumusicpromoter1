...
describe("LAMOU usage telemetry surface classification", () => {
  test("classifies governed owner/core surfaces", () => {
    ...
    expect(classifyLamouRoute("/core/problems/CASE-001").surfaceType).toBe("HYPOTHESIS_PROBLEM");
    expect(classifyLamouRoute("/core/health").surfaceType).toBe("INDICATOR_METRIC");
    expect(classifyLamouRoute("/core/observability").surfaceType).toBe("METRIC_OBSERVABILITY");
  ...
  test("classifies apps, mapa, client and installation surfaces", () => {
    ...
    expect(classifyLamouRoute("/owner/products/APP-001").surfaceType).toBe("PRODUCT_APP_CATALOG");
